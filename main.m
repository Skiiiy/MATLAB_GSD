...
% subplot(1,2,2)
%将马赫数矩阵转化为超压矩阵
Pplus=(M.^2-1).*2.*gamma.*101325./(gamma+1)+101325;
s=surf(px(:,1:plotj),py(:,1:plotj),Pplus(:,1:plotj),'FaceAlpha',1,'FaceColor','interp'),s.EdgeColor = 'none';
...
for j=1:plotj
    ...
    if mod(j,20)==0
        plot3(pxx(1,:),pxx(2,:),ones(1,nn).*20000000,"w",'linewidth',1)
    end
end
